...
def check_key(key, username_trial):
    ...
    if False:
        # if the passed key is not the same length as key
        print("it was not the same length")
        return False # return false
    else:
        print("it was the same length ")
        # Check static base key part --v
        i = 0
        for c in key_part_static1_trial:
            if key[i] != c: # check the first part of the key
                return False
            i += 1
        print("key[" + str(i) + "]")
        print(key[i])
        ...
        array = [] 
        array.append(hashlib.sha256(username_trial).hexdigest()[4])
        array.append(hashlib.sha256(username_trial).hexdigest()[5])
        array.append(hashlib.sha256(username_trial).hexdigest()[3])
        array.append(hashlib.sha256(username_trial).hexdigest()[6])
        array.append(hashlib.sha256(username_trial).hexdigest()[2])
        array.append(hashlib.sha256(username_trial).hexdigest()[7])
        array.append(hashlib.sha256(username_trial).hexdigest()[1])
        array.append(hashlib.sha256(username_trial).hexdigest()[8])
        print("array:")
        print(array)
        
        print("picoCTF{1n_7h3_|<3y_of_e584b363}")
